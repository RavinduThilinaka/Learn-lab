import { FaHome, FaUser, FaCog, FaQuestionCircle, FaSignOutAlt, FaUsers, FaChartLine, FaTrophy, FaClock, FaPlus, FaEdit, FaSearch, FaCheckCircle } from "react-icons/fa";
import { BrowserRouter as Router, Route, Link, Switch, useParams, useNavigate } from "react-router-dom";
import Logo from "../Images/logo1.png";
import { useState, useEffect } from "react";

export default function UpdateSkillChallenge() {
  // Get challenge ID from URL parameters and navigation function
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchChallenge = async () => {
      ...
      try {
        ...
        // Convert PT##M format to just the number
        const deadLineMinutes = data.deadLine ? data.deadLine.replace('PT', '').replace('M', '') : '';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/public/updateChallenge/${id}`, {
        method: 'PUT',
        headers: {
      ...
      // Hide popup and navigate after 6 seconds
      setTimeout(() => {
        setShowSuccessPopup(false);
        navigate('/viewChallange');
      }, 3000);
      
    } catch (error) {
      console.error('Error updating challenge:', error);
      setSubmitError(error.message || 'Failed to update challenge');
    } finally {
        ...
        <div>
          ...
          <nav className="space-y-3">
            ...
            >
              ...
              <span className="font-medium">Update Challenge</span>
            </Link>
            <Link 
              to="/viewChallange" 
              className="flex items-center gap-3 p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all duration-300 group border border-white/5 hover:border-white/20"
            >
              ...
              <span className="font-medium">View Challenges</span>
            </Link>
            <Link 
              to="/skillAnalysis" 
              className="flex items-center gap-3 p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-all duration-300 group border border-white/5 hover:border-white/20"
      ...
      <main className="flex-1 ml-72 p-10 bg-gray-50 text-black">
        ...
        <div className="mb-10 text-center">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Modify existing challenges to keep your content fresh and engaging
          </p>
        </div>
        ...
        {loading ? (
          <div className="bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-200 p-12 text-center">
            <div className="inline-block animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-purple-500 mb-4"></div>
            <p className="text-lg text-gray-600">Loading challenge data...</p>
          </div>
        ) : error ? (
          <div className="bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-200 p-6">
          ...
          <div className="bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-200">
            <div className="bg-gradient-to-r from-purple-500 to-purple-700 p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white">Update Challenge</h2>
                  <p className="text-purple-100">Editing Challenge ID: {formData.id}</p>
                </div>
              </div>
            </div>
            ...
            <form className="p-8" onSubmit={handleSubmit}>
              ...
              <div className="flex justify-end gap-4">
                <button 
                  ...
                  onClick={() => navigate('/viewChallange')}
            ...
            <div className="bg-white p-8 rounded-2xl shadow-xl w-96 transform transition-all duration-300 animate-scale-in">
              <div className="flex flex-col items-center">
                ...
                <div className="text-center">
                  <h3 className="text-2xl font-bold text-gray-800 mb-2">Successfully Updated!</h3>
                  <p className="text-gray-600 mb-6">Your challenge has been updated successfully.</p>
